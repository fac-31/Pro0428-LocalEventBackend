import { Event, FullEvent, FrequencyObject } from '../models/event.model.ts';
...
export function createEventKey(TF_IDR_Object: {[key: string]: {[key: string]: number}}, eventsV1: FullEvent[]) {
  ...
  eventsV1.forEach((event) => {
    ...
    if (eventTerms) {
      const validTerms = Object.keys(eventTerms).filter(term => term.length >= 3);
...
export function calculateTermFrequency(frequencyObject: FrequencyObject) {
  ...
  frequencyObject.names.forEach((name) => {
    ...
    terms.forEach((term) => {
      ...
      const docCount: number = frequencyObject.allTerms.filter(allTerm => allTerm === term).length;
      const inverseDocumentFrequency: number = frequencyObject.names.length / docCount;
